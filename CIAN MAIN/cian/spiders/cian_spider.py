...
from cian.constants import fields
...
class CianSpider(scrapy.Spider):
    ...
    i = 1
    start_urls = [f'https://www.cian.ru/cat.php?deal_type=sale&engine_version=2&offer_type=flat&p={i}&region=1']
    ...
    def parse(self, response: HtmlResponse):
        ...
        yield response.follow(f'https://www.cian.ru/cat.php?deal_type=sale&engine_version=2&offer_type=flat&p={self.i}&region=1',meta={'dont_redirect': True,"handle_httpstatus_list": [302]}, callback=self.parse)
    ...
    def parse_page(self, response: HtmlResponse):
        ...
        time = [i for i in time if i != ' ' and i != '⋅' and 'откроется' not in i]
        print(time, stations)
        # for i in range(len(time)):
        #     if 'откроется' in time[i]:
        #         del time[i], stations[i]
    ...
    def parse_stations(self, stations, times):
        ...
        if stations != []:
            if times == []:
                return None, stations[0]
            if 'Аминьевское шоссе' in stations:
                del stations[stations.index('Аминьевское шоссе')]
                del times[stations.index('Аминьевское шоссе')]
